...
// Braço do instrumento em SVG. Cordas na vertical (grave→agudo, esq→dir), dedos em linhas.
// Cada casa (corda × dedo) é clicável e é a forma de responder nos modos de treino.

import { motion } from "framer-motion";
import { Cell, cellKey, cellsForPosition, Instrument, getPosition } from "@/lib/music/instruments";
import { Naming, shortName } from "@/lib/music/notes";
...
const H = 470;
const NUT_Y = 46;
const ROW_Y = [46, 140, 212, 278, 340]; // dedos 0..4
const STR_LEFT = 66;
const STR_RIGHT = 234;
...
export default function Fingerboard({
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-[310px] select-none drop-shadow-sm" role="group" aria-label="Braço do instrumento">
      ...
      {/* nomes das cordas soltas, acima da pestana */}
      ...
      {cells.map((c) => {
        ...
        const label = showNames ? shortName(c.midi, naming) : c.finger === 0 ? "0" : String(c.finger);
        ...
        return (
          <motion.g
            ...
            whileHover={disabled || !inPool ? undefined : { scale: 1.12 }}
            animate={isSel || isReveal ? { scale: [1, 1.25, 1] } : { scale: 1 }}
            transition={{ duration: 0.35 }}
          >
            <circle cx={cx} cy={cy} r={17} fill={fill} stroke={stroke} strokeWidth={isSel || isReveal ? 3 : 1.5} opacity={inPool ? 1 : 0.4} />
            <text x={cx} y={cy + 5} textAnchor="middle" fontSize={showNames ? 12 : 14} fontWeight={700} fill={textFill} opacity={inPool ? 1 : 0.5}>
              {label}
            </text>
          </motion.g>
        );
      ...
      {/* rótulo da posição */}
      <text x={W / 2} y={H - 12} textAnchor="middle" fill="#9aa1ad" fontSize="12">
        {pos.label}
      </text>
    </svg>
  );
